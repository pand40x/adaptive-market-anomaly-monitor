...
### 2026-05-28 20. Audit Fixes and Recalibration

Audited the anomaly algorithm end to end and fixed the main calibration/session risks. Same-direction alerts are now deduped only inside the configured follow-through window, so a fresh later anomaly is not accidentally suppressed. Market rhythm now respects NYSE and Turkish holidays, BIST normal and half-day sessions, US half days, forex weekend close/open, and CME-style commodity futures maintenance breaks. Forex volume weight is forced to zero because Yahoo FX volume is not reliable. Calibration grids now differ by asset class and use each class's natural movement size and follow-up horizon. Active anomaly storage now keeps only the latest follow-up set from the current scan, while historical alerts and evaluations remain in their own collections. After recalibration on 3-year history, selected class settings were: crypto score 5.0 / 4.0% follow-through / 6 alerts / 66.67% precision; stock score 3.4 / 5.0% / 4 alerts / 100% precision; index score 2.9 / 2.5% / 2 alerts / 50% precision; forex score 1.8 / 0.5% / 6 alerts / 83.33% precision; commodity score 3.2 / 3.0% / 2 alerts / 100% precision; BIST score 3.2 / 5.0% / 8 alerts / 100% precision. Test suite passed with 31 tests.

### 2026-05-28 21. GitHub Push and Dokploy Deploy

Committed and pushed the audit fixes to GitHub on `main` with commit `43faffc`. Ran `market-watch anomaly-benchmark --years 3`, `market-watch scan`, `market-watch active-anomalies`, and `market-watch evaluate-alerts` after calibration. Dokploy accepted the deploy request and the latest deployment finished with status `done`. No secrets were printed.
